...
class Template:
    ...
    def encode_oneturn(
        ...
        """
        if self.name == 'BianQue':
            query = messages[-2]['content']
            history = messages[:-2]
            model_history = []
            for i in range(0,len(history),2):
                model_history.append((history[i]['content'],history[i+1]['content']))
            
            if len(model_history) == 0:
                prompt = query
            else:
                prompt = ""
                for i, (old_query, response) in enumerate(model_history):
                    prompt += "[Round {}]\n问：{}\n答：{}\n".format(i, old_query, response)
                prompt += "[Round {}]\n问：{}\n答：".format(len(model_history), query)
...
_register_template(
    name="BenTaso",
    format_user=StringFormatter(slots=["下面是一个问题，运用医学知识来正确回答提问.\n{{content}}\n### 回答：\n"]),
...
_register_template(
    name="Huatuo",
    format_user=StringFormatter(slots=["<问>：{{content}}\n<答>："]),
    format_system=StringFormatter(slots=[{"token": "[gMASK]"}, {"token": "sop"}, "{{content}}"]),
    format_separator=EmptyFormatter(slots=["\n\n"]),
    efficient_eos=True,
)
...
_register_template(
    name="mmedlm",
    format_user=StringFormatter(slots=["Below is an instruction that describes a task. Write a response that appropriately completes the request.\n\n### Instruction:\n{{content}}\n\n### Response:"]),
    format_system=StringFormatter(slots=[{"token": "[gMASK]"}, {"token": "sop"}, "{{content}}"]),
    format_separator=EmptyFormatter(slots=["\n\n"]),
    efficient_eos=True,
)
...
_register_template(
    name="BianQue",
    format_user=StringFormatter(slots=["病人：{{content}}\n医生：",{"token": "[gMASK]"},{"token":"<sop>"}]),
    format_system=StringFormatter(slots=[{"token": "[gMASK]"}, {"token": "sop"}, "{{content}}"]),
    format_separator=EmptyFormatter(slots=["\n\n"]),
    efficient_eos=True,
)
...
_register_template(
    ...
    # 不设置default_system，就不会加上system字段
    default_system="请用中文回答",
